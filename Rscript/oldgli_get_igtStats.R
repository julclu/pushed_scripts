    ...
    if ("roi_analysis" %in% tnum_files){
      ...
      if (igt_stat_csv %in% svk_csv_files){
        ...
        if((dim(igt_stats)[1]==1 & dim(igt_stats)[2]==1)
           | dim(igt_stats)[1]>5){
          data = data
          print(paste(i, bnum_tnum_df$bnum[i], bnum_tnum_df$tnum[i], "igt_stats_failure", sep="_"))
        } else{
          igt_stats = igt_stats[,1:7]
          colnames(igt_stats)=c("tnum","sfnum","vialID","index","X.CEL","X.NEL","X.NEC")
          data = rbind(data, igt_stats)
          print(paste(i, bnum_tnum_df$bnum[i], bnum_tnum_df$tnum[i], "success", sep="_"))
        }
      } else{
        data=data
        print(paste(i, bnum_tnum_df$bnum[i], bnum_tnum_df$tnum[i], "failure b/c igt_stat file not in svk_roi_analysis", sep = "_"))
      }
    } else{
      data=data
      print(paste(i, bnum_tnum_df$bnum[i], bnum_tnum_df$tnum[i], "failure b/c no svk_roi_analysis folder", sep = "_"))
    }
  }
  return(data)
}
