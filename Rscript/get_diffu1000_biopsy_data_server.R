...
get_diffu1000_biopsy = function(bnum_tnum_df, measure, headerlabel=NULL, datatype=NULL){
  ...
  if(is.na(headerlabel) && !is.na(datatype)){
    if(datatype == "adcfa"){
      get_diffu1000_biopsy_noheader(bnum_tnum_df, measure)
    }
    else{
      get_ev1000_biopsy_noheader(bnum_tnum_df, measure)
    }
  }
  ...
  ## want only specific headerlabel for adcfa
    ...
    get_diffu1000_biopsy_withheader(bnum_tnum_df, measure, headerlabel)
  }
  ## want all the adcfa and all of the ev data 
  else if (is.na(headerlabel) && is.na(datatype)){
    data_adcfa = get_diffu1000_biopsy_noheader(bnum_tnum_df, measure)
    data_ev = get_ev1000_biopsy_noheader(bnum_tnum_df, measure) 
