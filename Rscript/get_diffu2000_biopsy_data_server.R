...
get_diffu2000_biopsy_withheader=function(bnum_tnum_df, measure, headerlabel){
  data=data.frame()
  for(i in 1:dim(bnum_tnum_df)[1]){
    ## here we are reading in the csv file from the first element in bnum_tnum_df 
    #y=paste("/data/RECglioma/", bnum_tnum_df$bnum[i], "/", bnum_tnum_df$tnum[i], "/svk_roi_analysis/", bnum_tnum_df$tnum[i],  "_roi_adcfa2000.csv", sep="")
    ...
    if ("svk_roi_analysis" %in% tnum_files && "roi_analysis" %in% tnum_files){
      ...
      diffu_csv = paste(bnum_tnum_df$tnum[i],  "_roi_adcfa2000.csv", sep="")
      ## only if the csv we're looking for exists do we move forward with extracting the data 
      if (diffu_csv %in% svk_files){
        tnum_roi_adcfa2000=read.table(diffu_csv, sep=",", header=T)
        
        ## here we are going to get the measure block we are looking for 
        tnum_roi_adcfa2000_measure = tnum_roi_adcfa2000[grep(measure, as.character(tnum_roi_adcfa2000$measure)),]
        
        ## here we are going to calculate the number of biopsy just from this data 
        nrow = dim(tnum_roi_adcfa2000_measure)[1]
        
        ## here we are going to extract only the biopsy information rows 
        
        tnum_roi_adcfa2000_measure = data.frame(tnum_roi_adcfa2000_measure)
        if ("nawm" %in% tnum_roi_adcfa2000_measure$roi.label[1:10]){
          tnum_roi_adcfa2000_m_biopsy = tnum_roi_adcfa2000_measure[12:nrow,]
        }
        else {
          tnum_roi_adcfa2000_m_biopsy = tnum_roi_adcfa2000_measure[11:nrow,]
        }
        ...
        if(headerlabel %in% colnames(tnum_roi_adcfa2000_m_biopsy)){
          exactheaderlabel=paste("^", headerlabel,"$", sep="")
          h=grep(exactheaderlabel, colnames(tnum_roi_adcfa2000_m_biopsy))
          tnum_roi_adcfa2000_m_r_headl= data.frame(tnum_roi_adcfa2000_m_biopsy[,1:4], headerlabel = tnum_roi_adcfa2000_m_biopsy[,h]) 
          colnames(tnum_roi_adcfa2000_m_r_headl)=c(colnames(tnum_roi_adcfa2000_m_r_headl)[1:4], headerlabel)
        }
        else{
          tnum_roi_adcfa2000_m_r_headl= data.frame(tnum_roi_adcfa2000_m_biopsy[,1:4], headerlabel = NA) 
          colnames(tnum_roi_adcfa2000_m_r_headl)=c(colnames(tnum_roi_adcfa2000_m_r_headl)[1:4], headerlabel)
        }
...
get_diffu2000_biopsy_noheader=function(bnum_tnum_df, measure){
  data=data.frame()
  for(i in 1:dim(bnum_tnum_df)[1]){
    #y=paste("/data/RECglioma/", bnum_tnum_df$bnum[i], "/", bnum_tnum_df$tnum[i], "/svk_roi_analysis/", bnum_tnum_df$tnum[i],  "_roi_adcfa2000.csv", sep="")
    #tnum_roi_adcfa2000=read.table(y, sep=",", header=T)
    y=paste("/data/RECglioma/", bnum_tnum_df$bnum[i], "/", bnum_tnum_df$tnum[i], sep="")
    tnum_files = list.files(y)
    if ("svk_roi_analysis" %in% tnum_files && "roi_analysis" %in% tnum_files){
      setwd(paste(y, "/svk_roi_analysis", sep=""))
      ...
      if (diffu_csv %in% svk_files){
        ...
        if (dim(tnum_roi_adcfa2000_m_biopsy)[2]>9){
          tnum_roi_adcfa2000_m_biopsy_tmp = data.frame(tnum_roi_adcfa2000_m_biopsy[,1:5], adc=tnum_roi_adcfa2000_m_biopsy$adc, fa=tnum_roi_adcfa2000_m_biopsy$fa, nadc= tnum_roi_adcfa2000_m_biopsy$nadc, nfa=tnum_roi_adcfa2000_m_biopsy$nfa)
          tnum_roi_adcfa2000_m_biopsy=tnum_roi_adcfa2000_m_biopsy_tmp
        }
        data = bind_rows(data, tnum_roi_adcfa2000_m_biopsy)
      }
      else{
        data = data
      }
...
#######################################################################################################################################
get_ev2000_biopsy_noheader=function(bnum_tnum_df, measure){
  data=data.frame()
  for(i in 1:dim(bnum_tnum_df)[1]){
    #y=paste("/data/RECglioma/", bnum_tnum_df$bnum[i], "/", bnum_tnum_df$tnum[i], "/svk_roi_analysis/", bnum_tnum_df$tnum[i],  "_roi_ev1ev2ev32000.csv", sep="")
    y=paste("/data/RECglioma/", bnum_tnum_df$bnum[i], "/", bnum_tnum_df$tnum[i], sep="")
    tnum_files = list.files(y)
    if ("svk_roi_analysis" %in% tnum_files && "roi_analysis" %in% tnum_files){
      setwd(paste(y, "/svk_roi_analysis", sep=""))
      svk_files=list.files(pattern="csv")
      ev_csv = paste(bnum_tnum_df$tnum[i],  "_roi_ev1ev2ev32000.csv", sep="")
      if (ev_csv %in% svk_files){
        tnum_roi_ev1ev2ev32000=read.table(ev_csv, sep=",", header=T)
        
        ## here we are going to get the measure block we are looking for 
        tnum_roi_ev1ev2ev32000_measure = tnum_roi_ev1ev2ev32000[grep(measure, as.character(tnum_roi_ev1ev2ev32000$measure)),]
        
        ## here we are going to calculate the number of biopsy just from this data 
        nrow = dim(tnum_roi_ev1ev2ev32000_measure)[1]
        
        ## here we are going to extract only the biopsy information rows 
        
        tnum_roi_ev1ev2ev32000_measure = data.frame(tnum_roi_ev1ev2ev32000_measure)
        if ("nawm" %in% tnum_roi_ev1ev2ev32000_measure$roi.label[1:10]){
          tnum_roi_ev1ev2ev32000_m_biopsy = tnum_roi_ev1ev2ev32000_measure[8:nrow,]
        }
        else {
        ...
        if (dim(tnum_roi_ev1ev2ev32000_m_biopsy)[2]>13){
          tnum_roi_ev1ev2ev32000_m_biopsy_tmp = data.frame(tnum_roi_ev1ev2ev32000_m_biopsy[,1:5], ev1=tnum_roi_ev1ev2ev32000_m_biopsy$ev1, 
                                                           ...
                                                           evrad=tnum_roi_ev1ev2ev32000_m_biopsy$evrad, nev1=tnum_roi_ev1ev2ev32000_m_biopsy$nev1, 
                                                           nev2=tnum_roi_ev1ev2ev32000_m_biopsy$nev2, nev3=tnum_roi_ev1ev2ev32000_m_biopsy$nev3, 
                                                           nevrad=tnum_roi_ev1ev2ev32000_m_biopsy$nevrad)
...
#######################################################################################################################################
#######################################################################################################################################
get_ev2000_biopsy_withheader=function(bnum_tnum_df, measure, headerlabel){
  data=data.frame()
  for(i in 1:dim(bnum_tnum_df)[1]){
    y=paste("/data/RECglioma/", bnum_tnum_df$bnum[i], "/", bnum_tnum_df$tnum[i], sep="")
    tnum_files = list.files(y)
...
get_diffu2000_biopsy = function(bnum_tnum_df, measure, headerlabel=NULL, datatype=NULL){
  ...
  if(is.na(headerlabel) && !is.na(datatype)){
    if(datatype == "adcfa"){
      get_diffu2000_biopsy_noheader(bnum_tnum_df, measure)
    }
    else{
      get_ev2000_biopsy_noheader(bnum_tnum_df, measure)
    }
  }
  ...
  ## want only specific headerlabel for adcfa
    ...
    get_diffu2000_biopsy_withheader(bnum_tnum_df, measure, headerlabel)
  }
  ## want all the adcfa and all of the ev data 
  else if (is.na(headerlabel) && is.na(datatype)){
    data_adcfa = get_diffu2000_biopsy_noheader(bnum_tnum_df, measure)
    data_ev = get_ev2000_biopsy_noheader(bnum_tnum_df, measure) 
