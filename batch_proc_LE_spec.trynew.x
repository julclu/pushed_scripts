...
## here you need to get the specific line 
	...
	## here i'm going to try to add a caveat; that if already processed then don't need to keep going 

	if (-e svk_roi_analysis/${tnum}_${seo}_fbhsvdfcomb_biopsy_empcsal_normxs_sinc.tab) then 
		@ i = i + 1
		echo "already processed; moving onto next"
	else 
		cd /data/bioe2/*/${bnum}/${tnum}
		set Enum = `ls -d E*`
		echo 'Enum set'
